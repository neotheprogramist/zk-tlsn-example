...
contract BatchMintGate {
    ...
    constructor(
        ...
        VERIFIER = IRecursiveVerifier(verifier_);
    ...
    function batchMint(bytes calldata proof, bytes32[] calldata publicInputs, address recipient) external {
        ...
        if (nullVkHash != EXPECTED_NULL_VK_HASH || recursiveVkHash != EXPECTED_RECURSIVE_VK_HASH) {
            revert InvalidVkHashes(EXPECTED_NULL_VK_HASH, nullVkHash, EXPECTED_RECURSIVE_VK_HASH, recursiveVkHash);
        }
        ...
        emit BatchMintClaimed(msg.sender, recipient, transfersRoot, totalAmount);
    }
}
