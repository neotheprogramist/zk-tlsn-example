...
use tlsn::{
    ...
    config::{
        prove::ProveConfig, prover::ProverConfig as TlsnProverConfig, tls::TlsClientConfig,
...
impl Prover {
        ...
        S: AsyncRead + AsyncWrite + Send + Unpin + 'static,
    {
        ...
        let (mpc_tls_connection, prover_fut, session_handle, verifier_io_rx) = setup_and_connect(
            runtime,
            tls_client_config,
            tls_commit_config,
            verifier_socket,
        ...
        let prover_output = generate_and_finalize_proof(prover, &prove_config).await?;
    ...
    S: AsyncRead + AsyncWrite + Send + Unpin + 'static,
{
    ...
    runtime.spawn_detached(Box::pin(async move {
        ...
        // dropped, the prover has already returned an error upstream and the
...
fn build_prove_config(
    ...
    transcript_commitment_builder.default_kind(TranscriptCommitmentKind::Hash { alg: hash_alg });
