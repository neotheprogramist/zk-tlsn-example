...
use crate::{
    ...
    repo_root,
    zk::{
        cli::{self, VerifierTarget, cleanup_cli_outputs, read_field_words_from_bytes},
        prove::{KeccakProof, Proof, extract_transfer_fields_from_proof},
    },
};
...
pub(crate) fn parse_hex_field_word(value: &str) -> Result<[u8; HONK_FIELD_BYTES]> {
    let hex = value.strip_prefix("0x").ok_or(Error::InvalidInput {
        context: "field word",
...
// ─── Recursive settlement aggregation ──────────────────────────────────────
