...
use crate::server::app::{ServerConfigResponse, TransferRequest};
use alloy::{
...
use crate::{
    chain::{
        OnchainContracts, connect_anvil, load_local_deployment, load_settlement_artifacts,
        mint_amount, preflight_vk_check, read_claimed_root, read_token_balance, repo_root,
        submit_proof_onchain,
    },
    client::{connect_and_attest, open_client_endpoint},
    demo::{DemoConnectionConfig, create_transfer, fetch_server_config},
};
...
async fn submit_and_verify_settlement(
    ...
    let contracts = load_local_deployment(&repo_root, &provider, settlement.state.to_user_id)
        .await
        .context("failed to load predeployed settlement contracts; run `cargo run --release --bin fixture` after starting Anvil")?;
