...
import { assertEquals, sleep, spawnCargoBinary, waitForTcp } from "./lib/run-binary.mjs";
  ...
  return new Promise((resolve, reject) => {
    ...
    page.on("console", (msg) => {
      ...
      if (text.startsWith("ZKTLSN_ERROR ")) {
        finish(null, new Error(`page reported error: ${text.slice("ZKTLSN_ERROR ".length)}`));
      }
    });
...
try {
  const wasmPath = path.join(process.cwd(), "demo/assets/wasm/core_bg.wasm");
