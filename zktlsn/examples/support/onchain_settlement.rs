...
use crate::deployment_artifacts::{
    self, PreparedSettlementArtifacts, SettlementDeployment, load_embedded_artifacts,
};

pub const DEFAULT_ANVIL_RPC_URL: &str = "http://127.0.0.1:8545";
...
) -> Result<PreparedSettlementArtifacts> {
    let artifacts = load_embedded_artifacts()?.settlement;
    ensure!(
        artifacts.verification_key == proof.verification_key,
        "embedded settlement deployment artifacts do not match the generated settlement verification key; rerun `cargo run --package zktlsn --release --example fixture`"
    );
    Ok(artifacts)
}
    ...
    P: Provider,
{
    ...
    let gate = deploy_bytecode(
        provider,
        append_constructor_args(
            &artifacts.settlement_mint_gate_bytecode,
    ...
    P: Provider,
{
    ...
    deployment_artifacts::write_local_settlement_deployment(repo_root, &deployment)
...
where
    P: Provider,
{
    let deployment = deployment_artifacts::load_local_settlement_deployment(repo_root)?
