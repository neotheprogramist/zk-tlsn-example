...
use crate::{
    ...
    stwo_proof::run_stwo_proof_flow,
};
...
struct Cli {
    ...
    server_name: String,
    #[arg(long, env = "ZKTLSN_VERIFIER_ADDR", default_value = "[::1]:5001")]
...
async fn run(cli: Cli) -> Result<()> {
    ...
    let flow = connect_and_prove(&cli, transfer.tx_id)
        .await
...
async fn connect_and_prove(
    cli: &Cli,
    tx_id: u64,
) -> Result<stwo_proof::StwoProofFlow> {
